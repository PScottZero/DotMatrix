...
class GB_CPU {
	...
	// instance data
	const int INST_COUNT = 100000;
	uint16_t AF, BC, DE, HL, PC, SP;
	std::array<uint8_t, 0xFFFF> mem;
	std::array<uint8_t, 0x200000> cart;
	bool IME;
	...
	void loadCartridge() {
		ifstream file("C:/Users/8psco/Documents/Roms/GB/Tetris.gb", ios::in | ios::binary | ios::ate);
		char* buf;
		if (file.is_open()) {
